...
static __thread __attribute__((tls_model("initial-exec"))) char*  mp = NULL; /* Default memory pool. */
...
{
    unsigned long sz = 0;
    const char *sz_s = getenv("COMMON_ALLOCATOR_SIZE_MB");
    if (sz_s) {
        sz = atoi(sz_s);
    }
    if (!sz) {
    ...
    void* pool = malloc(sz);
#ifdef __linux__
    madvise(pool, sz, MADV_POPULATE_WRITE);
#endif
    init_memory_pool(sz, pool);
}
